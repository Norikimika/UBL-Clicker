        ...
        try:
            ...
            while True:
                if time.time() - request_loop_start > max_wait_seconds:
                    print("[Timeout] Exceeded 5 minutes waiting for request approval")
                    telegram("<b>✹ Unlock Bootloader</b>\n<i>-> Exceeded 5 minutes waiting for request approval.</i>")
                    exit()
                ...
                try:
                    ...
                    if code == 0:
                        ...
                        if apply_result == 1:
                            ...
                            telegram(f"<b>✹ Unlock Bootloader</b>\n<i>-> Request not submitted, request limit reached, try again at {deadline_format} (Month/Day).</i>")
                            exit()
                        elif apply_result == 4:
                            deadline_format = data.get("deadline_format", "Not specified")
                            print(f"[Status] Request not submitted, blocked from submitting requests until {deadline_format} (Month/Day).")
                            telegram(f"<b>✹ Unlock Bootloader</b>\n<i>-> Request not submitted, blocked from submitting requests until {deadline_format} (Month/Day).</i>")
                            exit()
                    elif code == 100001:
